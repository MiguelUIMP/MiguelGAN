...
class WGAN_trainer:
    ...
    def plot_samples(self, plot_options, num_model, label="FINAL", process=True):
        ...
        for data_t in data_type:
            ...
            compare_mean = compare_df.mean(axis=0).round(2).apply(round,args=((2)))
            compare_std = compare_df.std(axis=0).round(2).apply(round,args=((2)))
            compare_skew = compare_df.skew(axis=0).round(2).apply(round,args=((2)))
            compare_kurtosis = compare_df.kurtosis(axis=0).round(2).apply(round,args=((2)))
